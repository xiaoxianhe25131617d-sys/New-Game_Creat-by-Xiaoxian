...
- 风格：精细像素，偏梦境心理冒险；地上世界是当前唯一可玩地图，不再规划地下区域素材。
- 色彩：柔和草绿、蓝灰远景、暖木色建筑、低饱和高光；避免大面积纯灰、重复圆圈纹理和临时纯色块。
- 可读性：玩家、NPC、拾取物、谜题机关要比背景亮一档；交互物必须有稳定轮廓和小范围高光。
- 素材落地：最终项目资源放入 `assets/` 或 `map/`，运行时代码不引用临时生成目录。
